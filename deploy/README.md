...
Optional Nutrition check-in push reminders use Firebase Web Messaging and FCM HTTP v1. They remain disabled while the Cloud Build substitution `_NUTRITION_FCM_PROJECT_ID` is `__not_configured__`; set it to the Firebase project id only after configuring the web app and granting the Nutrition Cloud Run service account permission to send FCM messages. In Vercel, supply the matching public `VITE_FIREBASE_API_KEY`, `VITE_FIREBASE_AUTH_DOMAIN`, `VITE_FIREBASE_PROJECT_ID`, `VITE_FIREBASE_MESSAGING_SENDER_ID`, `VITE_FIREBASE_APP_ID`, and `VITE_FIREBASE_VAPID_KEY` values, then rebuild the web app. These are browser configuration values, not server credentials. Do not enable the feature unless client and server values refer to the same Firebase project. Run `deploy/setup-nutrition-check-in-push.ps1` with `-ApiOrigin` and the protected scheduler token to create the five-minute dispatch job. Users opt in per device and separately choose one weekly weekday, time, and IANA time zone. The notification text is always generic; disabling the schedule stops reminders, while deleting a device subscription revokes that browser token. The server only sends within 30 minutes of the selected local time and does not replay missed days.
