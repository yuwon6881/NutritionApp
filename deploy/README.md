...
Optional Google Health weight, nutrition, and body-fat uploads are sent while their user is active: each `/api/integrations/google-health/sync` call (Dashboard and Progress, at most every two minutes) first spends up to six seconds on that user's due uploads, when the API and database are already awake. One hourly Cloud Scheduler POST to `/internal/google-health-sync`, authenticated with the same protected header, sweeps everything else — retries, records queued just before the app closed, and leases left by an interrupted run — so the API and database can scale to zero between runs. Without that job, work queued outside an active session never leaves the server. Run `deploy/setup-google-health-sync.ps1` with `-ApiOrigin` and a token supplied from Secret Manager or `NUTRITION_CLEANUP_TOKEN`; the script creates or updates the dedicated job idempotently. Each queue processes at most 25 due records or 45 seconds per sweep and leaves the rest for the next one. The OAuth web client must have the health-metrics write scope approved before live testing.
