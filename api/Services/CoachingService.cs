...
public sealed class CoachingService(AppDb db)
{
    public async Task<CoachingPreview> Preview(CancellationToken ct)
    {
        ...
        var weekStart = CheckInWeek.WeekStart(today);
        var declined = await db.CheckIns.AnyAsync(d => d.WeekStart == weekStart && d.Decision == "declined" && !d.Deleted, ct);
        var newWeek = last != null && weekStart > CheckInWeek.WeekStart(last.Date);
        ...
        var due = last == null || last.ProfileRevision != user.ProfileRevision || (newWeek && !declined) || completionDue;
        var nextCheckIn = last == null ? weekStart : CheckInWeek.NextCheckIn(last.Date);
        ...
        return new(user.Revision, result, result.Eligible && due, hold, weekStart, nextCheckIn, changes, result.Evidence);
    }
