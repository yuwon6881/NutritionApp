...
internal sealed record GoogleHealthOperationResult(
    bool Done,
    bool Success,
...
internal sealed class GoogleHealthWeightProviderException(
    string category,
    string message,
    bool transient = false,
    bool unknownCreate = false,
...
internal static class GoogleHealthWeightProvider
{
    public static Task<GoogleHealthOperationResult> CreateAsync(HttpClient http, string token, GoogleHealthWeightDataPoint data, CancellationToken ct)
        => SendOperationAsync(http, HttpMethod.Post, "https://health.googleapis.com/v4/users/me/dataTypes/weight/dataPoints", token, BuildDataPoint(data), true, ct);

    public static Task<GoogleHealthOperationResult> UpdateAsync(HttpClient http, string token, string resourceName, GoogleHealthWeightDataPoint data, CancellationToken ct)
    ...
    public static Task<GoogleHealthOperationResult> DeleteAsync(HttpClient http, string token, string resourceName, CancellationToken ct)
        => SendOperationAsync(http, HttpMethod.Post, "https://health.googleapis.com/v4/users/me/dataTypes/weight/dataPoints:batchDelete", token, new { names = new[] { resourceName } }, false, ct, deleteRequest: true);

    private static object BuildDataPoint(GoogleHealthWeightDataPoint data)
    {
        var timestamp = DateTimeOffset.Parse(data.SampleTime, System.Globalization.CultureInfo.InvariantCulture);
    ...
    private static async Task<GoogleHealthOperationResult> SendOperationAsync(HttpClient http, HttpMethod method, string url, string token, object body, bool create, CancellationToken ct, bool deleteRequest = false)
    {
        ...
        catch (GoogleHealthWeightProviderException)
        {
            throw;
        }
        catch (HttpRequestException) when (create)
        {
            throw new GoogleHealthWeightProviderException("upload_status_unknown", "The create response was lost; check Google Health before retrying.", unknownCreate: true);
        }
        catch (JsonException) when (create)
        {
            throw new GoogleHealthWeightProviderException("upload_status_unknown", "The create response was unreadable; check Google Health before retrying.", unknownCreate: true);
        }
    ...
    private static async Task<HttpResponseMessage> SendAsync(HttpClient http, HttpRequestMessage request, bool create, CancellationToken ct, bool allowMissing = false)
    {
        ...
        response.Dispose();
        if (authentication) throw new GoogleHealthWeightProviderException("reconnect_required", "Reconnect Google Health to resume weight uploads.", authenticationFailure: true);
        if (transient) throw new GoogleHealthWeightProviderException("provider_unavailable", "Google Health temporarily rejected the upload.", true, retryAfter: retryAfter);
        throw new GoogleHealthWeightProviderException("provider_validation", SanitizeProviderMessage(body));
    }
    ...
    private static async Task<GoogleHealthOperationResult> ParseOperationAsync(HttpResponseMessage response, string? fallbackName, bool create, CancellationToken ct)
    {
        ...
        if (root.TryGetProperty("error", out var error) && error.ValueKind == JsonValueKind.Object)
        {
            ...
            return new(true, false, operationName, AuthenticationFailure: authentication, Transient: transient, ErrorCategory: category, ErrorMessage: SanitizeProviderMessage(message));
        }
    ...
    private static string SanitizeProviderMessage(string? message)
        => string.IsNullOrWhiteSpace(message) ? "Google Health rejected this weight upload." : "Google Health rejected this weight upload.";
}
