...
public record FoodResult(string Name,double Calories,double? Protein,double? Fat,double? Carbs,double? Fiber,string Source,double ServingGrams=100);
public sealed class FoodSearchService(HttpClient http,IConfiguration config,IMemoryCache cache)
{
    ...
    public async Task<IReadOnlyList<FoodResult>> Search(string query,CancellationToken ct)
    {
        ...
        foreach(var food in json.RootElement.GetProperty("foods").EnumerateArray())
        {
            ...
            results.Add(new(food.GetProperty("description").GetString()!,calories,Nutrient(1003),Nutrient(1004),Nutrient(1005),Nutrient(1079),"USDA FDC "+food.GetProperty("fdcId").GetInt32()));
        }
    ...
    public async Task<FoodResult> Barcode(string code,CancellationToken ct)
    {
        ...
        using var request=new HttpRequestMessage(HttpMethod.Get,$"https://world.openfoodfacts.org/api/v2/product/{code}?fields=product_name,nutriments");
        ...
        var result=new FoodResult(product.GetProperty("product_name").GetString()??"Packaged food",N("energy-kcal")!.Value,N("proteins"),N("fat"),N("carbohydrates"),N("fiber"),"Open Food Facts / ODbL / "+code);
