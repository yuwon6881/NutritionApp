...
public static class CentralAuthEndpoints
{
    ...
    public static void MapCentralAuth(this WebApplication app)
    {
        app.MapGet("/api/auth/central/start", (HttpResponse response, IConfiguration config, [FromServices] IDataProtectionProvider protection, IHostEnvironment environment) =>
        {
            ...
            var protector = protection.CreateProtector("nutrition-fitness-account-oidc-state-v1");
            ...
            IConfiguration config, IHostEnvironment environment, CancellationToken ct) =>
        {
            ...
            if (connect)
            {
                ...
                var validated = await accessTokens.Require(bearer, WorkoutScope, ct);
            ...
            if (connect)
            {
                ...
                Validation.Require(local is not null && local.IdentitySubject == identitySubject,
                    "This central subject is not attached to the current Nutrition account. Use the guarded identity attach operation first.", 403);
            ...
            Validation.Require(user is not null, "This central account has not been attached to a Nutrition account yet.", 403);
            var session = await auth.CreateSession(user!.Id, ct);
            response.Cookies.Append("nutrition-session", session, new CookieOptions { HttpOnly = true, Secure = !environment.IsDevelopment(), SameSite = SameSiteMode.Lax, Path = "/", MaxAge = TimeSpan.FromDays(30), IsEssential = true });
            return Results.Redirect(state.ReturnUrl);
        });
    }
    ...
    private static (LoginState State, bool Connect) ReadState(HttpRequest request, IDataProtectionProvider protection)
    {
        ...
        try
        {
            ...
            var loginProtector = protection.CreateProtector("nutrition-fitness-account-oidc-state-v1");
    ...
    {
        if (expected is not { } expectedUser || string.IsNullOrWhiteSpace(request.Cookies["nutrition-session"])) return null;
        var hash = AuthService.Hash(request.Cookies["nutrition-session"]!);
