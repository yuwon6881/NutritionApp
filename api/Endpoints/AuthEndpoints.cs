...
public static class AuthEndpoints
{
    public static void MapAuth(this WebApplication app)
    {
        ...
        {
            var user=await db.Users.SingleAsync(u=>u.Id==db.CurrentUser,ct); return new { user.Id,user.Username };
        });
        app.MapPost("/api/auth/logout",async(AppDb db,HttpContext http,CancellationToken ct) =>
        {
            var hash=AuthService.Hash(http.Request.Cookies["nutrition-session"]??"");
