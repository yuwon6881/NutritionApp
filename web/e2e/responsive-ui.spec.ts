...
test.beforeAll(async({request})=>{
  ...
  await save('entry',{date:current,name:'Grilled chicken with brown rice and vegetables',calories:640,protein:46,carbs:68,fat:18,fiber:9,quantity:1,unit:'serving',time:'12:30'});
...
for(const width of [390,768,1440])for(const theme of ['light','dark']){
  test(`${theme} ${width}: navigation, pages and entry surfaces`,async({page,context})=>{
    ...
    for(const name of ['Dashboard','Food Log','Progress','Coach','Settings']){
      ...
      if(name==='Food Log'){
        await expect(page.locator('[data-time-row="08:00"] .food-macro-summary')).toHaveAttribute('aria-label','Macros: Protein 28 g, Carbs 52 g, Fat 12 g');
