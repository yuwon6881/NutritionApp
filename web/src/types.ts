...
export type Profile={age:number;dateOfBirth:string|null;heightCm:number;weightKg:number;sex:'male'|'female';activity:number;goal:'lose'|'maintain'|'gain';maintenance:number|null;proteinGrams:number|null;resistanceTraining:boolean;pregnancyOrBreastfeeding:boolean;medicalNutrition:boolean;timeZone:string;phaseMode?:'open'|'duration'|'weight';phaseStart?:string|null;durationWeeks?:number|null;targetWeightKg?:number|null;phaseStartWeightKg?:number|null;energyAdjustmentPercent?:number|null;proteinPercent?:number|null;carbsPercent?:number|null;fatPercent?:number|null;macroPreset?:string|null};
