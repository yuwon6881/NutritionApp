...
export async function api<T>(path:string,body?:unknown):Promise<T>{
  const response=await fetch('/api'+path,{credentials:'same-origin',cache:'no-store',headers:{'Content-Type':'application/json','X-Nutrition-Request':'1'},...(body===undefined?{}:{method:'POST',body:JSON.stringify(body)})});
