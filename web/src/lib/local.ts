...
export async function wipeLocal(user:string){const db=await database();return new Promise<void>((resolve,reject)=>{const tx=db.transaction('accounts','readwrite');tx.objectStore('accounts').delete(user);tx.oncomplete=()=>resolve();tx.onerror=()=>reject(tx.error);});}
