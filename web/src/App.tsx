...
import {Utensils,Plus,Scale,Camera,ChartNoAxesCombined,Compass,Settings as SettingsIcon} from 'lucide-react';
...
type Page='today'|'progress'|'coach'|'settings';
...
function Workspace({user,onLogout}:{user:string;onLogout:()=>Promise<void>}){
  ...
  const openFood=(selectedDate:string,entry?:Entry,ai=false,restoreFocus?:HTMLElement|null)=>{
    setFoodDate(selectedDate);setFoodEditing(entry);setFoodInitialAi(ai);setFoodReturnFocus(restoreFocus??null);setFoodOriginPage(page);setFoodOpen(true);
  };
  ...
  const addOptions:ActionSheetOption[]=[
    {id:'food',label:'Log food',description:'Choose a saved food or enter a meal.',icon:<Utensils size={20}/>,onClick:()=>openFood(page==='today'?date:activeDate,undefined,false,addReturnFocus)},
    {id:'weight',label:'Log weight',description:'Record today’s scale weight.',icon:<Scale size={20}/>,onClick:()=>openWeight(page==='today'?date:activeDate,undefined,addReturnFocus)},
    {id:'scan',label:'Scan food or label',description:'Use a meal photo or nutrition label.',icon:<Camera size={20}/>,onClick:()=>openFood(page==='today'?date:activeDate,undefined,true,addReturnFocus)},
  ];
  const navigate=(next:Page)=>{setPage(next);window.scrollTo({top:0,behavior:'instant'});};
  const nav=[
  ...
  return <div className="app-shell">
    <aside className="sidebar">
      ...
      <nav aria-label="Main navigation">
        <div className="nav-mobile-items">
          <Button disabled={needsProfile} variant="tertiary" className={page==='today'?'nav-active':''} aria-current={page==='today'?'page':undefined} onClick={()=>navigate('today')}><Utensils size={20}/><span>Today</span></Button>
          <Button disabled={needsProfile} variant="tertiary" className={page==='progress'?'nav-active':''} aria-current={page==='progress'?'page':undefined} onClick={()=>navigate('progress')}><ChartNoAxesCombined size={20}/><span>Progress</span></Button>
          ...
          <Button variant="tertiary" className={page==='coach'?'nav-active':''} aria-current={page==='coach'?'page':undefined} onClick={()=>navigate('coach')}><Compass size={20}/><span>Coach</span></Button>
          <Button disabled={needsProfile} variant="tertiary" className={page==='settings'?'nav-active':''} aria-current={page==='settings'?'page':undefined} onClick={()=>navigate('settings')}><SettingsIcon size={20}/><span>Settings</span></Button>
        </div>
        <div className="nav-desktop-items">
          ...
          {nav.map(item=>{const Icon=item.icon;return <Button key={item.id} disabled={needsProfile&&item.id!=='coach'} variant="tertiary" className={page===item.id?'nav-active':''} aria-current={page===item.id?'page':undefined} onClick={()=>navigate(item.id)}><Icon size={21}/><span>{item.label}</span></Button>;})}
        </div>
      </nav>
    </aside>
    <main className="main-content">
      ...
      {!store.state?<section className="panel skeleton" aria-busy="true"><h1>Opening your diary…</h1><Button onClick={()=>void onLogout()}>Back to sign in</Button></section>:!needsProfile&&page==='today'?<Today store={store} date={date} setDate={setDate} onLog={()=>openFood(date)} onCoach={()=>navigate('coach')} onEdit={entry=>openFood(entry.date,entry)} onWeight={trigger=>openWeight(date,undefined,trigger)} onCopyDay={openCopy}/>:!needsProfile&&page==='progress'?<Progress store={store}/>:needsProfile||page==='coach'?<Coach store={store} onboarding={needsProfile}/>:<Settings store={store} onLogout={onLogout}/>}
      {store.state?.profile&&<MissedDays store={store}/>}
      {store.state&&<>
        <LogFood key={`${foodDate}:${foodEditing?.id??'new'}:${foodInitialAi?'scan':'food'}`} open={foodOpen} store={store} date={foodDate} editing={foodEditing} initialAi={foodInitialAi} restoreFocus={foodReturnFocus} onClose={()=>{setFoodOpen(false);setPage(foodOriginPage);}} onSaved={()=>{setFoodOpen(false);setDate(foodDate);setPage('today');}}/>
