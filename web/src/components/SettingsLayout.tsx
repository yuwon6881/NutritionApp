...
/** A titled settings group. `card` wraps the rows in one panel; `plain` lets the group hold its own cards. */
export function SettingsSection({id,title,description,icon:Icon,layout='card',children}:{
  id:string;
  title:string;
  ...
  return <section id={id} className="settings-section" aria-labelledby={`${id}-title`}>
...
function useActiveSection(ids:string[]){
  ...
  const key=ids.join('|');
  const choose=useCallback((id:string)=>{pinned.current=true;setActive(id);},[]);
  useEffect(()=>{
  ...
  useEffect(()=>{
    const sections=key.split('|').map(id=>document.getElementById(id)).filter((node):node is HTMLElement=>Boolean(node));
