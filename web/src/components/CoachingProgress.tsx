...
export function CoachingProgress({store}:{store:Nourish}){
  ...
  return <section className="panel"><h2>Your adaptive maintenance estimate</h2><p>Estimated total daily energy expenditure (TDEE), inferred from logged intake and weight. This is not a direct measurement of resting metabolism.</p>
    {pending?<p className="notice">Sync your changes to update goal progress and the next coaching preview.</p>:result&&<><h3>{number(result.expenditure)} kcal/day</h3><p className="source">Current coaching preview. Your diary keeps the accepted target until you approve a change.</p>{result.goalProgress&&<GoalSummary progress={result.goalProgress}/>}</>}
