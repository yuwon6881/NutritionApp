...
export function CoachingProgress({store}:{store:Nourish}){
  ...
  return <section className="panel">
    <div className="section-heading"><div><h2>Your adaptive maintenance estimate</h2></div></div>
    {pending?<p className="notice">Sync your changes to update goal progress and the next coaching preview.</p>:result&&<><h3>{number(result.expenditure)} kcal/day</h3>{result.goalProgress&&<GoalSummary progress={result.goalProgress}/>}</>}
    {points.length>0&&<><svg viewBox="0 0 700 220" className="weight-chart" role="img" aria-label={`Accepted maintenance estimates from ${number(points[0].result.expenditure)} to ${number(points.at(-1)!.result.expenditure)} calories per day.`}><line x1="55" y1="175" x2="670" y2="175" className="chart-grid"/><polyline className="trend-line" points={points.map(xy).join(' ')}/>{points.map(p=>{const [x,y]=xy(p).split(',');return <circle key={p.revision} cx={x} cy={y} r="3" className="trend-dot"><title>{p.date}: {number(p.result.expenditure)} kcal/day</title></circle>;})}<text x="50" y="45" textAnchor="end">{number(max)}</text><text x="50" y="175" textAnchor="end">{number(min)}</text><text x="55" y="208">{points[0].date}</text><text x="670" y="208" textAnchor="end">{points.at(-1)!.date}</text></svg><details><summary>Accepted maintenance history</summary>{points.map(p=><p key={p.revision}>{p.date}: {number(p.result.expenditure)} kcal/day</p>)}</details></>}
    <details className="chart-details"><summary>About adaptive expenditure & TDEE</summary><p className="source">Estimated total daily energy expenditure (TDEE), inferred from logged intake and weight. This is not a direct measurement of resting metabolism. Your diary keeps the accepted target until you approve a change.</p></details>
    {error&&<p className="source">Connect to refresh this coaching preview. Your saved history remains available.</p>}
  </section>;
}
