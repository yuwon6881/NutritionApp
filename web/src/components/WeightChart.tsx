...
export function WeightChart({series,weightUnit='kg'}:{series:ProgressWeightPoint[];weightUnit?:WeightUnit}){
  ...
  const values=series.flatMap(point=>view==='daily'?[point.scaleKg]:view==='trend'?[point.trendKg]:[point.scaleKg,point.trendKg]);
  ...
  const points=(field:'scaleKg'|'trendKg')=>series.map(point=>`${x(point.date)},${y(point[field])}`).join(' ');
  return <section className="panel"><div className="section-heading"><div><h2>Weight</h2></div>
    ...
    {series.length?<><svg ref={chart.ref} viewBox={`0 0 ${chart.width} 220`} className="weight-chart" role="img" aria-label={`${view==='both'?'Daily and trend':view==='daily'?'Daily scale':'Trend'} weight chart across ${series.length} points. Values are also available in the table below.`}>
    ...
    {view!=='trend'&&<><polyline points={points('scaleKg')} className="scale-line"/>{series.map(point=><circle key={`scale-${point.date}`} cx={x(point.date)} cy={y(point.scaleKg)} r="2.5" className="scale-dot"><title>{point.date}: {displayWeight(point.scaleKg,weightUnit,2)} {weightLabel(weightUnit)} scale weight</title></circle>)}</>}
    {view!=='daily'&&<><polyline points={points('trendKg')} className="trend-line"/>{series.map(point=><circle key={`trend-${point.date}`} cx={x(point.date)} cy={y(point.trendKg)} r="2" className="trend-dot"><title>{point.date}: {displayWeight(point.trendKg,weightUnit,2)} {weightLabel(weightUnit)} trend weight</title></circle>)}</>}
    <text x={chart.left} y="208">{series[0].date}</text><text x={chart.right} y="208" textAnchor="end">{series.at(-1)!.date}</text></svg><p className="chart-key">Muted: scale weight · Accent: trend weight · {weightLabel(weightUnit)}</p>
    <details><summary>Values as a table</summary><div className="table-scroll"><table><thead><tr><th>Date</th><th>Daily {weightLabel(weightUnit)}</th><th>Trend {weightLabel(weightUnit)}</th></tr></thead><tbody>{series.map(point=><tr key={point.date}><td>{point.date}</td><td>{displayWeight(point.scaleKg,weightUnit,2)}</td><td>{displayWeight(point.trendKg,weightUnit,2)}</td></tr>)}</tbody></table></div></details></>:<div className="empty"><h3>No weigh-ins yet</h3></div>}
  </section>;
}
