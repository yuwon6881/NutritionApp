...
export function WeightChart({series,weightUnit='kg'}:{series:ProgressWeightPoint[];weightUnit?:WeightUnit}){
  ...
  return <section className="panel"><div className="section-heading"><div><h2>Weight</h2></div>
    ...
    <p id={readoutId} className="chart-readout" aria-live="polite">
      ...
      {view!=='daily'&&<span>Trend {displayWeight(selected.trendKg,weightUnit,2)} {unit}</span>}
    </p>
    ...
    {view!=='trend'&&<><polyline points={points('scaleKg')} className="scale-line"/>{series.map(point=><circle key={`scale-${point.date}`} cx={x(point.date)} cy={y(point.scaleKg)} r={point===selected?5:2.5} className="scale-dot"><title>{point.date}: {displayWeight(point.scaleKg,weightUnit,2)} {unit} scale weight</title></circle>)}</>}
    {view!=='daily'&&<><polyline points={points('trendKg')} className="trend-line"/>{series.map(point=><circle key={`trend-${point.date}`} cx={x(point.date)} cy={y(point.trendKg)} r={point===selected?5:2} className="trend-dot"><title>{point.date}: {displayWeight(point.trendKg,weightUnit,2)} {unit} trend weight</title></circle>)}</>}
    ...
    <details><summary>Values as a table</summary><div className="table-scroll"><table><thead><tr><th>Date</th><th>Daily {unit}</th><th>Trend {unit}</th></tr></thead><tbody>{series.map(point=><tr key={point.date}><td>{point.date}</td><td>{displayWeight(point.scaleKg,weightUnit,2)}</td><td>{displayWeight(point.trendKg,weightUnit,2)}</td></tr>)}</tbody></table></div></details></>:<div className="empty"><h3>No weigh-ins yet</h3></div>}
  </section>;
}
