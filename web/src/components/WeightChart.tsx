...
export function WeightChart({weights,smoothed}:{weights:Point[];smoothed:Point[]}){
  ...
  return <section className="panel"><div className="section-heading"><div><h2>Weight</h2></div>
    ...
    <text x="55" y="208">{weights[0].date}</text><text x="670" y="208" textAnchor="end">{weights.at(-1)!.date}</text></svg><p className="chart-key">Muted: scale weight · Accent: calculated trend · kg</p>
    <details><summary>Values as a table</summary><div className="table-scroll"><table><thead><tr><th>Date</th><th>Daily kg</th><th>Calculated kg</th></tr></thead><tbody>{weights.map((p,i)=><tr key={p.date}><td>{p.date}</td><td>{number(p.kg,2)}</td><td>{number(smoothed[i]?.kg,2)}</td></tr>)}</tbody></table></div></details></>:<div className="empty"><h3>No weigh-ins yet</h3></div>}
  </section>;
}
