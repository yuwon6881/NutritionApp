...
export function EnergyBalance({store,period,summary,error,onPeriodChange}:{store:Nourish;period:ProgressPeriod;summary?:ProgressSummary;error?:string;onPeriodChange:(period:ProgressPeriod)=>void}){
  ...
  return <section className="panel energy-history"><div className="section-heading"><div><h2>Energy balance</h2><p>{summary?`${summary.start} to ${summary.end}`:'Loading the selected period…'}</p></div><SelectField label="Energy history period" value={period} onChange={value=>onPeriodChange(value as ProgressPeriod)}>
    ...
    {summary&&<>
      ...
      <h3>Intake and estimated maintenance</h3><svg ref={chart.ref} viewBox={`0 0 ${chart.width} 225`} className="weight-chart" role="img" aria-label={`Energy intake and estimated maintenance bars, grouped ${grouping}, from ${summary.start} to ${summary.end}. Exact values and incomplete days are in the table.`}>
      ...
      <details><summary>Values as a table</summary><div className="table-scroll"><table><thead><tr><th>Bucket date</th><th>Through</th><th>Intake</th><th>Maintenance</th><th>Balance</th><th>Status</th></tr></thead><tbody>{rows.map(row=><tr key={`${row.date}-${row.end}`}><td>{row.date}</td><td>{row.end}</td><td>{displayEnergy(row.intake,units.energy)} {energyUnit}</td><td>{displayEnergy(row.maintenance,units.energy)} {energyUnit}</td><td>{row.balance==null?'Unknown':`${row.balance>0?'+':''}${displayEnergy(row.balance,units.energy)} ${energyUnit}`}</td><td>{row.complete?'Complete':'Incomplete'}</td></tr>)}</tbody></table></div></details>
    </>}
  </section>;
}
