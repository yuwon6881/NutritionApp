...
export function EnergyBalance({store}:{store:Nourish}){
  ...
  return <section className="panel energy-history"><div className="section-heading"><div><h2>Energy balance</h2><p>{start} to {end}</p></div></div>
    ...
    {history.loading&&<p className="notice loading-status" role="status" aria-busy="true">Loading energy history…</p>}
    {!history.state&&!history.loading&&!history.error&&<p className="loading-status" role="status">Loading energy history…</p>}
    {history.state&&<>
