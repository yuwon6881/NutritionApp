...
Progress projects retained and queued scale weights immediately, including All history and profile-local date changes. Trend values affected by pending edits remain unknown: charts show gaps and readouts/tables show Pending, never scale weight substituted as a trend. Only server reconciliation restores those calculated values.

Explicit modal Close dismisses the whole flow after dirty confirmation. Back/Escape may return through clean inner steps when the flow provides cancellation navigation; an existing food-entry editor closes instead. Dirty confirmation preserves the original dismissal intent, and focus returns to the launcher when the dialog closes.
...
- **Workout refresh feedback**: Keep cached summaries visible during a refresh and announce the refresh through a quiet accessible status. Initial connected loads use a matching skeleton; provider failures use shared card feedback.
...
Notification Settings uses one reminder enable action to request permission, subscribe the device, and save the schedule. Valid schedule edits save automatically in order; device-only subscription controls remain secondary. Partial registration/save failures show `CardFeedback` with recovery and space from adjoining settings rows. Food logging methods occupy equal columns with centered labels; saved-food creation actions retain whole labels and wrap by action rather than by word.
