...
- Food calendars scroll independently of date selection; desktop arrows move one week, while tap/date picker/Today select dates. Calorie perimeter progress uses dated accepted targets and projected intake, caps at 100%, and exposes unknown values honestly. Use `DiaryEmptyState` for factual empty/read-only context and an available logging action.
- Notification reminder enablement combines device consent/subscription and account schedule saving; serialize automatic schedule edits and show partial failures through spaced `CardFeedback`.
- Food timeline cards use an ultra-compact two-line layout without source strings; entry actions use one accessible action sheet for Edit, Copy, Move to, and Delete; copy/move destination dialogs must preserve the entry snapshot, keep date/time validation explicit, and make deletion immediate but undoable through the held outbox mutation and `UndoToast`. Multi-select supports touch hold (400 ms) and toolbar selection, undoable bulk deletion, bulk move with original time preservation, and clipboard copying to paste into specific timeline slots or viewing dates.
...
- The mobile shell uses the existing `Modal`/`ActionSheet` and semantic tokens for safe-area-aware app bars, bottom navigation, bottom-sheet grab handles, touch scrolling, and Android/browser Back that unwinds popovers, dialog steps, selection mode, dialogs, then pages (`useBackLayer` for surfaces lighter than a `Modal`). Hover-only styling is gated by `@media (hover:hover)`, raw number inputs carry `inputMode`, and popovers dismiss on `pointerdown`; `check:standards` enforces all three. The Capacitor Android shell keeps the bundled interface available offline, routes API requests through the same production origin, and follows the active Ayu theme in its status bar. Preserve keyboard Escape, dirty-close confirmation, protected dialogs, focus restoration, and reduced-motion behavior when extending it.
- Progress projects pending scale-weight additions, edits, moves, and deletions across the selected range, including All history. Unknown downstream trend values are gaps with Pending readouts/table cells. Never use scale weight as a calculated trend. Re-evaluate ranges on the profile-local date boundary.
- Modal Close dismisses after dirty confirmation; Back/Escape can unwind new-food steps through the shared cancellation callback, while existing-entry editors close and restore focus. Preserve dismissal intent through discard confirmation.
- Cached training refreshes retain content and announce loading through an accessible status; initial connected loads show a skeleton.
- Chart scrubbing uses `useChartScrub` for touch and pointer inspection with a crosshair and value readout card, keeping keyboard arrow stepping and data table alternatives accessible.
- Tactile feedback uses `web/src/lib/haptics.ts` (`hapticTick`) for key interactions (card lift, selection, log commit, barcode detect, swipe threshold) and respects reduced motion and user preference.
- External integrations (Google Health and Workout) reuse shared components (Card, Button, Modal, Checkbox, CardFeedback), hold step data strictly in runtime memory (never IndexedDB/localStorage), provide pre-connect disclosure modals with the Google API Limited Use statement, expose independent connection and weight-stream status in Settings, surface generic temporary Workout sync warnings without discarding cached completed summaries, and render discrete missing-data gaps and known-day averages in Progress charts. Step day markers must support click and keyboard selection, while Settings `Sync now` must bypass the short server cache and expose loading or provider-warning feedback. Weight sync must explain new-entry eligibility, profile-local noon/offset, background processing, retained Google copies, and explicit unknown-upload recovery.
...
- Google Health step synchronization showing fresh/stale/unavailable badges, showing a compact Dashboard steps insight only while connected and after the calorie/macro summary, hiding it while disconnected, reconnect-required, or unresolved, preserving unknown days as discrete gaps in charts without guessing zeros, and never feeding step metrics into caloric or coaching algorithms;
- Google Health weight synchronization showing disabled, permission-limited, pending, reconnect-required, failed, and unknown-upload states independently from steps; preserving local saves during provider failure; exposing recovery only for actionable failures; and never changing local weights, trends, expenditure, or coaching;
...
| Food timeline entry actions | Browser assertions for the action sheet and swipe actions, date/time copy and move, undoable delete (Undo restores the same entry; an unchallenged delete persists), focus restoration, and retained mutation behavior; unit coverage for bulk selection, bulk move/delete, clipboard mutations, and slot pasting |
...
| Touch gestures, Back, or on-screen keyboard | A `*.touch.spec.ts` in the `mobile-chromium` project using the real-touch helpers in `e2e/helpers/touch.ts` |
...
| Google Health integration | Unit tests for in-memory sync manager, known-day averaging, date lookups, weight grams/noon conversion, and preference state; API integration tests for OAuth partial consent, same-identity reconnect, KMS encryption, queue transaction/idempotency, lease/retry/unknown handling, multi-tenant isolation, disconnect cleanup, and export redaction; browser coverage for disclosure, permission-limited settings, recovery, focus restoration, and responsive/theme states |
