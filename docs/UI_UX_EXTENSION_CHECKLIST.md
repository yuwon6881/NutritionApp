...
- External integrations (Google Health and Workout) reuse shared components (Card, Button, Modal, Checkbox, CardFeedback), hold step data strictly in runtime memory (never IndexedDB/localStorage), provide pre-connect disclosure modals with the Google API Limited Use statement, expose independent connection and weight-stream status in Settings, surface generic temporary Workout sync warnings without discarding cached completed summaries, and render discrete missing-data gaps and known-day averages in Progress charts. Weight sync must explain new-entry eligibility, profile-local noon/offset, background processing, retained Google copies, and explicit unknown-upload recovery.
